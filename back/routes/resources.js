...
const { addResource, getResources, deleteResource, updateResource, getResourceById, searchResources } = require("../Controllers/resourceController");
...
router.post("/:id/like", checkAuth, async (req, res) => {
  try {
    ...
    if (isLiked) {
      // Remove like
      resource.likes.pull(userId);  // Use Mongoose's pull method
      resource.likesCount--;
    } else {
      // Add like
      resource.likes.push(userId);  // Use Mongoose's push method
      resource.likesCount++;
    }
...
router.post("/:id/favorite", async (req, res) => {
  try {
    ...
    if (isFavorited) {
      ...
      resource.favorites.pull(userId);
