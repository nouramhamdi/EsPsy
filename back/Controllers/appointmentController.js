...
module.exports.getRequestedAppointments = async (req, res) => {
  ...
  exports.getUserAppointments = async (req, res) => {
    try {
        ...
        .populate('psychologist', 'username email ') // Populate psychologist details
  ...
  try {
    ...
    const htmlBody = `
      <div style="font-family: Arial, sans-serif; padding: 20px; line-height: 1.6;">
        ...
        <p>Bonjour,</p>
        <p>Votre psychologue a planifié un rendez-vous pour vous.</p>
        <p><strong>Date:</strong> ${new Date(date).toLocaleDateString('fr-FR')}</p>
        <p><strong>Heure:</strong> ${time}</p>
        <p>Si ce créneau vous convient, veuillez confirmer votre rendez-vous :</p>
        <a href="${confirmationLink}" style="background-color: #4CAF50; color: white; padding: 10px 15px; text-decoration: none; border-radius: 5px; display: inline-block; margin-right: 10px;">Confirmer</a>
        <a href="${rescheduleLink}" style="background-color: #FF9800; color: white; padding: 10px 15px; text-decoration: none; border-radius: 5px; display: inline-block;">Reprogrammer</a>
        <p>Cordialement,</p>
...
module.exports.checkAppointmentsForReminders = async () => {
  try {
    ...
    const upcomingAppointments = await Appointment.find({
      ...
      scheduledDate: {
        $gte: new Date(reminderTime.getTime() - 60000), // 1 minute buffer
        $lt: new Date(reminderTime.getTime() + 60000) // 1 minute buffer
      }
...
exports.createFile = async (req, res) => {
  try {
    ...
    const newFile = await FilePatient.create({ user, psychologist, note });
...
// Update note by file ID
exports.updateFileNote = async (req, res) => {
  try {
    const { note } = req.body;


    const file = await FilePatient.findByIdAndUpdate(
      req.params.id,
      { note },
