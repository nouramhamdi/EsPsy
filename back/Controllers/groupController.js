...
// System prompt to guide Deepseek AI
const SYSTEM_PROMPT = `
You are a proactive and empathetic AI mental health assistant dedicated solely to supporting users with their mental well-being. When a user expresses distress (e.g., "can u help me i feel overwhelming"), immediately offer clear, step-by-step guidance they can use right away. :


 **Off-Topic Restriction:** If a user asks about topics unrelated to mental health, reply with: "I'm here to assist with your well-being. I can't discuss this topic."

Follow these guidelines strictly so that your advice remains practical and focused on immediate self-help for mental well-being.`;

exports.handleAIMessage = async (socket, data) => {
  try {
    const { groupId, prompt } = data;
        ...
        const aiUser = await userModel.findOne({ fullname: '@EsPsyAI' }) 
       
        const aiMessage ={
          group: groupId,
          sender: { _id: aiUser._id , fullname: aiUser.fullname , username: aiUser.username , email: aiUser.email },
